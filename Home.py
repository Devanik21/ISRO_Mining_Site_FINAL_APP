...
user_query = st.text_area("Pose your question to the Cosmic Oracle:", 
                          placeholder="e.g., 'What are the prospects for Helium-3 mining on Luna's far side?', 'Tell me about site AS-1872', or 'What is the average resource value in the current map view?'", 
                          height=120, key="oracle_query")
if st.button("Seek Cosmic Insight", key="oracle_button"):
    if user_query:
        with st.spinner("Oracle is contemplating the cosmic echoes..."):
            time.sleep(1.5)
            query_lower = user_query.lower()
            responded_to_data_query = False

            # Check for queries about recommended sites
            for site_id_iter in recommendation_df['Site ID']: # Use a different variable name
                if site_id_iter.lower() in query_lower:
                    site_info = recommendation_df[recommendation_df['Site ID'] == site_id_iter].iloc[0]
                    st.info(f"🔮 Oracle's Insight on {site_id_iter}:\n"
                            f"- Location: {site_info['Location']}\n"
                            f"- Primary Resource: {site_info['Primary Resource']}\n"
                            f"- Value Index: {site_info['Value Index']}\n"
                            f"- Extraction Difficulty: {site_info['Extraction Difficulty']}\n"
                            f"- Mission Duration: {site_info['Mission Duration']}")
                    responded_to_data_query = True
                    break
            
            if not responded_to_data_query:
                # Check for queries about map data (using current distance_filter)
                if "map" in query_lower or "galactic resource" in query_lower or "current view" in query_lower or "star data" in query_lower:
                    # Re-filter star_data based on the current distance_filter from the sidebar
                    current_map_data_for_oracle = star_data.copy() # Start with full data
                    if distance_filter == "< 100 LY":
                        current_map_data_for_oracle = star_data[star_data['distance'] < 100]
                    elif distance_filter == "100-500 LY":
                        current_map_data_for_oracle = star_data[(star_data['distance'] >= 100) & (star_data['distance'] < 500)]
                    elif distance_filter == "500-1000 LY":
                        current_map_data_for_oracle = star_data[(star_data['distance'] >= 500) & (star_data['distance'] < 1000)]
                    elif distance_filter == "> 1000 LY": # Ensure this matches the selectbox option
                        current_map_data_for_oracle = star_data[star_data['distance'] >= 1000]

                    if not current_map_data_for_oracle.empty:
                        if "average resource value" in query_lower or "avg resource" in query_lower:
                            avg_val = current_map_data_for_oracle['resource_value'].mean()
                            st.info(f"🔮 Oracle's Insight: The average resource value in the map view for '{distance_filter}' is {avg_val:.2f}.")
                            responded_to_data_query = True
                        elif "how many sites" in query_lower or "count sites" in query_lower or "number of sites" in query_lower:
                            count = len(current_map_data_for_oracle)
                            st.info(f"🔮 Oracle's Insight: There are {count} sites displayed in the map view for '{distance_filter}'.")
                            responded_to_data_query = True
                        elif "most common site type" in query_lower:
                            common_type = current_map_data_for_oracle['site_type'].mode()[0] if not current_map_data_for_oracle['site_type'].mode().empty else "N/A"
                            st.info(f"🔮 Oracle's Insight: The most common site type in the map view for '{distance_filter}' is {common_type}.")
                            responded_to_data_query = True
                        else:
                            st.info("🔮 Oracle's Insight: For the map data, I can tell you about average resource value, site count, or common site types for the current distance filter. Please be more specific.")
                            responded_to_data_query = True 
                    else:
                        st.info(f"🔮 Oracle's Insight: No data matches the current map filter ('{distance_filter}') to provide specific insights.")
                        responded_to_data_query = True

            if not responded_to_data_query: # Fallback to general cosmic insights
                general_responses = [
                ...
                st.info(f"🔮 Oracle's Insight: {np.random.choice(general_responses)}")
    else:
