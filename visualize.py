...
def show_visualize_page():
    ...
    st.write(
        "Explore different visualizations to understand the dataset and the impact of user preferences."
    )
    ...
    if "final_score" not in data.columns:
        st.write(
            "The 'final_score' column does not exist, calculating it based on weights."
        )
    ...
    feature = st.selectbox(
        "Select Feature to Visualize", data.columns[1:]
    ...
    features = st.multiselect(
        "Select Features for Pairplot", data.columns[1:]
    )  # Exclude non-numeric columns if necessary
    if len(features) > 1:
        fig, ax = plt.subplots()
    ...
    aggregate_feature = st.selectbox(
        "Select Feature for Aggregate Barplot", data.columns[1:]
    )

    # Create a barplot of average feature values by celestial body
    fig, ax = plt.subplots()
    sns.barplot(
        x="Celestial Body", y=aggregate_feature, data=data, palette="coolwarm", ax=ax
    )
    ...
    st.write(
        top_sites_display.style.background_gradient(
            subset=["adjusted_score"], cmap="coolwarm"
        )
    )
