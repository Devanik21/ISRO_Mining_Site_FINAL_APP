...
def show_insights_page():
    ...
    st.write(
        f"**🌕 Number of Unique Celestial Bodies:** `{df['Celestial Body'].nunique()}`"
    )
    ...
    celestial_body_summary = (
        df_filtered.groupby("Celestial Body")
        .agg(
            {
                "iron": ["mean", "std"],
                "nickel": ["mean", "std"],
    ...
    min_value, max_value = st.slider(
        ...
        value=(
            float(df["Estimated Value (B USD)"].min()),
            float(df["Estimated Value (B USD)"].max()),
        ),
    )

    df_value_filtered = df_filtered[
        (df_filtered["Estimated Value (B USD)"] >= min_value)
        & (df_filtered["Estimated Value (B USD)"] <= max_value)
    ]

    # Insights by Estimated Value
    st.write("## 💰 Insights by Estimated Value")
    high_value_sites = df_value_filtered[
        df_value_filtered["Estimated Value (B USD)"]
        > df_value_filtered["Estimated Value (B USD)"].median()
    ]
    low_value_sites = df_value_filtered[
        df_value_filtered["Estimated Value (B USD)"]
        <= df_value_filtered["Estimated Value (B USD)"].median()
    ]
    ...
    st.dataframe(
        high_value_sites[
            ["Celestial Body", "iron", "nickel", "water_ice", "Estimated Value (B USD)"]
        ]
        .describe()
    ...
    st.markdown(
        "**For Higher Value Sites:** Focus on sites with **higher iron** and **nickel percentages**."
    )
    st.markdown(
        "**For Sustainable Mining:** Prioritize sites with **higher sustainability** and **efficiency indices**."
    )
    st.markdown(
        "**For Proximity:** Consider sites that are **closer to Earth** for easier access."
    )
