...
def show_analyze_page():
    ...
    st.write(
        "Analyze the characteristics of different mining sites with advanced data analytics."
    )
    ...
    combined_df = pd.concat(
        [df.select_dtypes(include=["float64", "int64"]), encoded_df], axis=1
    )
    ...
    # Bar Chart
    st.write("### 📊 Bar Chart")
    selected_bar_columns = st.selectbox(
        "Select a column for bar chart:", df.columns.tolist(), key="bar_chart_columns"
    )
    ...
    selected_hist_column = st.selectbox(
        "Select a column for histogram:", df.columns.tolist(), key="histogram_columns"
    )
    ...
    # Boxplot
    st.write("### 📦 Boxplot")
    selected_box_columns = st.selectbox(
    ...
    selected_violin_columns = st.selectbox(
        "Select a column for violin plot:",
        df.select_dtypes(include=["float64", "int64"]).columns.tolist(),
