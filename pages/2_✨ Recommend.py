...
def show_recommend_page():
    ...
    st.write(
        "Set your preferences in the sidebar, and our model will recommend the most suitable mining sites for your needs!"
    )
    ...
    other_minerals = (
        st.sidebar.slider("Other Minerals Importance (%)", 0, 100, 50) / 100.0
    )
    sustainability_efficiency = (
        st.sidebar.slider("Sustainability/Efficiency Importance (%)", 0, 100, 50)
        / 100.0
    )
    distance_from_earth = (
        st.sidebar.slider("Distance from Earth Importance (%)", 0, 100, 50) / 100.0
    )
    ...
    if st.button("Recommend"):
        ...
        st.markdown(
            "<div style='text-align: center;'>✨ Exploration successful: New cosmic mining opportunities discovered! ✨</div>",
            unsafe_allow_html=True,
        )
