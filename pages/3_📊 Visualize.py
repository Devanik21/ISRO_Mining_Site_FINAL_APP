...
def show_visualize_page():
    ...
    if scatter_x and scatter_y:
        ...
        plt.title(
            f"{scatter_x} vs. {scatter_y} Composition", fontsize=16, fontweight="bold"
        )
    ...
    if hist_col:
        ...
        plt.figure(figsize=(10, 6))
        sns.histplot(
            df[hist_col], kde=True, color="crimson", bins=20, edgecolor="black"
        )
    ...
    if violin_col:
        ...
        plt.title(
            f"{violin_col} Distribution by Celestial Body (Violin Plot)",
            fontsize=16,
            fontweight="bold",
        )
    ...
    selected_columns = st.multiselect(
        "Choose Columns for Correlation Heatmap", df.columns
    )
    if selected_columns:
    ...
    pairplot_columns = st.multiselect(
        "Choose Columns for Pairplot", df.columns, default=df.columns[:3]
    )
    if pairplot_columns:
        sns.pairplot(
            df[pairplot_columns],
        ...
        plt.suptitle(
            "Pairplot of Selected Features", y=1.02, fontsize=16, fontweight="bold"
        )
        st.pyplot(plt)
    ...
    if hexbin_x and hexbin_y:
        ...
        plt.figure(figsize=(10, 6))
        plt.hexbin(
            df[hexbin_x], df[hexbin_y], gridsize=30, cmap="Purples", edgecolors="black"
        )
        plt.colorbar(label="Count")
        plt.title(
            f"Hexbin Plot of {hexbin_x} vs {hexbin_y}", fontsize=16, fontweight="bold"
        )
        plt.grid(True)
        st.pyplot(plt)
